...
from transforms.comissoes import flatten_colegiado, flatten_mista, flatten_membro
from utils import configure_utf8, unwrap_list

configure_utf8()


def _get_senator_ids(client: SenateApiClient) -> list[str]:
    """Read senator IDs from the already-extracted Parquet file, or fetch fresh."""
    ...
    with SenateApiClient() as client:
        ...
        for i, senador_id in enumerate(senator_ids, 1):
            ...
            try:
                ...
                comissoes_membro = unwrap_list(
                    parlamentar.get("MembroComissoes", {}).get("Comissao")
                )
                membros = [flatten_membro(senador_id, c) for c in comissoes_membro]
